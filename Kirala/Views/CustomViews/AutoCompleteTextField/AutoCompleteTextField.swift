...
class AutoCompleteTextField: UITextField {
    ...
    var completionValues: [String] = defaultDomains
    var autoCompleteType: ATAutoCompleteType = .email {
        didSet {
    ...
    var autoCompletionTextColor: UIColor = .completionGray {
        didSet {
            autoCompleteLabel.textColor = autoCompletionTextColor
        }
    }
    var autoCompletionFont: UIFont = .init() {
        didSet {
...
extension Selector {
    static let editingDidEnd = #selector(AutoCompleteTextField.editingDidEnd)
    static let editingChanged = #selector(AutoCompleteTextField.editingChanged)
}
