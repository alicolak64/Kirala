...
open class FastisController<Value: FastisValue>: UIViewController, JTACMonthViewDelegate, JTACMonthViewDataSource {
    ...
    private func configureCell(_ cell: JTACDayCell, forItemAt date: Date, cellState: CellState, indexPath: IndexPath) {
        ...
        if let cachedConfig = self.viewConfigs[indexPath] {
            ...
            var newConfig = DayCell.makeViewConfig(
                ...
                calendar: self.config.calendar,
                closedRanges: self.closedRanges
            )
    ...
    public func calendar(
        ...
        if let value = self.value as? FastisRange {
            var newFromDate = value.fromDate
            if value.fromDate.isInSameDay(in: self.config.calendar, date: value.toDate) {
                newFromDate = value.toDate
            }
            var newValues = FastisRange(from: newFromDate, to: date)
            if newValues.fromDate > newValues.toDate {
                newValues = FastisRange(from: date, to: newFromDate)
            }
            
            if newValues.isRangeContained(closedRanges: self.closedRanges) {
                return false
            }
