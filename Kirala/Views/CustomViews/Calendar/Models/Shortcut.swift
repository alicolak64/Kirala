...
public extension FastisShortcut where Value == FastisRange {
    ...
    static var nextWeek: FastisShortcut {
        FastisShortcut(name: "Next week") { calendar in
            let now = Date()
            let weekAgo = calendar.date(byAdding: .day, value: 6, to: now)!
            return FastisRange(from: now.startOfDay(in: calendar), to: weekAgo.endOfDay(in: calendar))
        }
    }
    ...
    static var nextMonth: FastisShortcut {
        FastisShortcut(name: "Next month") { calendar in
            let now = Date()
            let monthAgo = calendar.date(byAdding: .month, value: 1, to: now)!
            return FastisRange(from: now.startOfDay(in: calendar), to: monthAgo.endOfDay(in: calendar))
        }
    }
